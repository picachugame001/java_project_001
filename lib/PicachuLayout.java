...
public class PicachuLayout implements LayoutManager, java.io.Serializable {
	...
	*/
	  synchronized (target.getTreeLock()) {
		...
		int maxwidth = 300 - (insets.left + insets.right + hgap*2);
        int nmembers = target.getComponentCount();
        ...
        for (int i = 0 ; i < nmembers ; i++) {
            ...
            if (m.isVisible()) {
                ...
                if ((x == 0) || ((x + d.width) <= maxwidth)) {
                    ...
                    rowh = moveComponentsTest(target, insets.left + hgap, y,
		...
		System.out.printf("rowh = %d\n",rowh);
		System.out.printf("start = %d\n",start);
        //moveComponentsTest(target, insets.left + hgap, y, maxwidth - x, rowh,
        //               start, nmembers, ltr, useBaseline, ascent, descent);
		return new Dimension(300,y+rowh);
      }
      ...
      synchronized (target.getTreeLock()) {
        ...
        for (int i = 0 ; i < nmembers ; i++) {
            ...
            if (m.isVisible()) {
                ...
                //dim.height = Math.max(dim.height, d.height);
				maxHeight_Current = Math.max(maxHeight_Current, d.height);
                if (firstVisibleComponent) {
                    firstVisibleComponent = false;
                } else {
                    //dim.width += hgap;
